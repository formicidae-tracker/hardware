...
void PIDSetConfig(PIDController *c, const ArkePIDConfig *config) {
	...
	if (c->config.IntegralMult == 0) {
		...
		c->integralOverflowMax =
		    ((c->maxCommand) << c->config.DividerPowerInt) /
		    c->config.IntegralMult;
		c->integralOverflowMax *= 120;
		c->integralOverflowMax /= 100;
		c->integralOverflowMin = -c->integralOverflowMax;
	}
}
...
int16_t PIDCompute(PIDController *c, uint16_t current, ArkeSystime_t ellapsed) {
	...
	int32_t res = (c->config.ProportionalMult * error) /
	              ((int32_t)1 << c->config.DividerPower);
	...
	if (c->proportionalOverflow == true) {
		...
		toAdd /= 1000;
		if ((c->integralError > 0) &&
		    (toAdd > (c->integralOverflowMax - c->integralError))) {
	...
	res += (c->config.DerivativeMult * derror) /
	       ((int32_t)1 << c->config.DividerPower);
	res += (c->config.IntegralMult * c->integralError) /
	       ((int32_t)1 << c->config.DividerPowerInt);
